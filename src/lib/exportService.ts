...
class ExportService {
  ...
  private async exportToPDF(results: AnalysisResult[], options: ExportOptions, metadata: ExportMetadata): Promise<void> {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.setFontSize(8);
      doc.text(
        `ScamShield Analysis Report - Page ${i} of ${pageCount}`,
        20,
        290
      );
      doc.text(
...
<head>
    ...
    <title>ScamShield Analysis Report - ${results.analysisId}</title>
    <style>
        body {
...
<body>
    <div class="header">
        <h1>🛡️ ScamShield Analysis Report</h1>
    ...
    <div class="footer">
        ...
        <p><strong>Generated by:</strong> ScamShield - Advanced Scam Detection System</p>
